...
class Config {
    ...
    function updatePref($var,&$errors) {
        ...
        $sql= 'UPDATE '.CONFIG_TABLE.' SET isonline='.db_input($var['isonline']).
            ...
            ',spoof_default_smtp='.db_input(($var['default_smtp_id'] && isset($var['spoof_default_smtp']))?1:0). //BSK Fixed bug by changing default_smtp to default_smtp_id
            ...
            ',html_email_template='.db_input($var['html_email_template']). // BSK Update HTML email template
            ' WHERE id='.$this->getId();
        //echo $sql;
        if(db_query($sql)) {
